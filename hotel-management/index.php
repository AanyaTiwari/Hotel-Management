...
//FOR DELETING ROWS WHOSE CHECKOUT DATE IS LESS THAN TODAY
$today = date("Y-m-d");
$sql="SELECT * FROM rooms WHERE checkout < '$today'";
$result= mysqli_query($user->db,$sql)  or die(mysqli_connect_errno()."Query failed");
                    
while($rows=mysqli_fetch_array($result))
{
    $sql2="DELETE FROM rooms WHERE checkout < '$today'";
    $result2= mysqli_query($user->db,$sql2)  or die(mysqli_connect_errno()."Query failed");
}
session_destroy();
?>
      ...
      <div class="row" style="color: #ffbb2b">
        <div class="col-md-4 wellfix">
            <h4><strong>Contact Us</strong></h4><hr>
              Address : AIGS<br>
              Mail : sagarkp.17.abca@acharya.ac.in <br>
        </div>
